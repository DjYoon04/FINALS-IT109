<template>
  <div class="bg-white rounded-3xl shadow-xl overflow-hidden h-full pb-16">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 max-h-full overflow-y-auto scrollbar-hide">
      ...
      <span class="ml-3 text-emerald-600 font-medium">Loading profile...</span>
      </div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            ...
            <div class="w-full">
              <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <p class="text-gray-600">{{ profile.specialization || 'Not specified' }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Skills Section -->
          <section class="mt-6">
            <h3 class="text-md font-medium text-emerald-700 mb-3">Skills</h3>
            <div v-if="profile.skills" class="flex flex-wrap gap-2">
              <span 
          ...
          <div v-if="reviews.length > 0" class="space-y-4">
            ...
            >
              <div class="flex justify-between items-center mb-2">
                <!-- Reviewer's info: icon, name, and stars -->
                <div class="flex items-center space-x-2">
                  <!-- CircleUser Icon -->
                  <CircleUserRound class="w-5 h-5 text-emerald-600" />
                  <!-- Display reviewer's name -->
                  <span class="text-sm font-semibold text-emerald-800">
                    {{ review.users_info?.fullname || 'Anonymous' }}
                  </span>
                  <!-- Display star rating -->
                  <div class="flex">
                    <Star 
                      ...
                      :class="[ 
                        'w-4 h-4', 
                        i <= review.reviews?.star ? 'text-yellow-400 fill-current' : 'text-gray-300'
                      ]"
                    />
                  </div>
                </div>
              ...
              <p class="mt-2 text-gray-600 text-sm">
                {{ review.reviews?.comments || 'No comment available' }}
              </p>
            </div>
          </div>
          <p 
            v-else 
            class="text-gray-500 italic text-center bg-gradient-to-br from-emerald-100 via-teal-200 to-cyan-200 shadow-md p-4 rounded-2xl"
          >
            No reviews yet.
          </p>
        </section>
...
import { Facebook, Mail, XIcon, Star, CircleUserRound } from 'lucide-vue-next'
...
const starRating = computed(() => {
  ...
  const total = reviews.value.reduce((acc, review) => acc + (review.reviews?.star || 0), 0)
  return Math.round((total / reviews.value.length) * 2) / 2 // Round to nearest 0.5
})
